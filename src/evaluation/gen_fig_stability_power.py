"""
Fig: Two-panel — SHAP Jaccard stability (left) + Power analysis curve (right)
"""
import matplotlib
matplotlib.use('Agg')
...
ax.axhline(0.634, color='#B71C1C', lw=2, ls='--', label='Observed AUROC = 0.634')
ax.axhline(0.630, color='#FF6F00', lw=1.5, ls=':', alpha=0.7, label='AVEC 2017 baseline = 0.630')
ax.axvline(35,   color='#B71C1C', lw=2,   ls='--')
ax.axvline(159,  color='#2E7D32', lw=1.8, ls='-.',
           label='Required N for observed effect = 159')

ax.annotate('N = 35\n(this study)', xy=(35, 0.76), xytext=(70, 0.78),
            arrowprops=dict(arrowstyle='->', color='#B71C1C', lw=1.5),
            fontsize=9, color='#B71C1C', fontweight='bold')
ax.annotate('N = 159\nrequired', xy=(159, 0.64), xytext=(200, 0.68),
...
fig.suptitle('Model Reliability Analysis: SHAP Stability and Statistical Power',
             fontsize=12, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(OUT/'fig_stability_power.png', dpi=200, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.close()
print('Saved -> fig_stability_power.png')
